...
class PointIntraPartOffsetHead(PointHeadTemplate):
    ...
    def get_loss(self, num_voxels_per_example, tb_dict=None):
        ...
        point_loss_cls, point_loss_cls_ew, tb_dict = self.get_cls_layer_loss(num_voxels_per_example, tb_dict)
        point_loss_part, point_loss_part_ew, tb_dict = self.get_part_layer_loss(num_voxels_per_example, tb_dict)
        point_loss = point_loss_cls + point_loss_part
        point_loss_ew = point_loss_cls_ew + point_loss_part_ew

        if self.box_layers is not None:
            point_loss_box, point_loss_box_ew, tb_dict = self.get_box_layer_loss(num_voxels_per_example, tb_dict)
            point_loss += point_loss_box
            point_loss_ew += point_loss_box_ew
        return point_loss, point_loss_ew, tb_dict
