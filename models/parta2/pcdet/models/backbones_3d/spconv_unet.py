...
class UNetV2(nn.Module):
    ...
    """
    def __init__(self, model_cfg, input_channels, grid_size, voxel_size, point_cloud_range, use_adversarial_discriminator, **kwargs):
        super().__init__()
...
class AdversarialDiscriminator(nn.Module):
    ...
    def __init__(self, model_cfg, input_channels, grid_size, voxel_size, point_cloud_range, **kwargs):
        ...
        self.gavgpool = torch.nn.AvgPool1d(64, stride = 1)
        self.fc1 = torch.nn.Linear(64, 1)

        self.frozen = False

    def grad_reverse(self, x):
        return GradReverseLayer.apply(x)#, self.unfrozen)
    ...
    def forward(self, batch_dict):
        ...
        x.features = self.grad_reverse(x.features)
        x = self.conv4(x)

        # pool/classify per example
        for k in range(batch_dict['batch_size']):
            example_x = x.features[torch.nonzero(x.indices[:,0]==k)][:,0,:]
            example_x = F.avg_pool1d(example_x.permute(1,0).unsqueeze(0), example_x.shape[0])[0,:,0]
            if k == 0:
...
class GradReverseLayer(torch.autograd.Function):
    '''
    Credit: https://discuss.pytorch.org/t/solved-reverse-gradients-in-backward-pass/3589/8
    '''
