...
class RoIHeadTemplate(nn.Module):
    ...
    def get_box_reg_layer_loss(self, forward_ret_dict):
        ...
        if loss_cfgs.REG_LOSS == 'smooth-l1':
            ...
            for batch_id in range(gt_boxes3d_ct.shape[0]):            
                ind0, ind1 = gt_boxes3d_ct.shape[1] * batch_id, gt_boxes3d_ct.shape[1] * (batch_id +1)
                loss_ew = (rcnn_loss_reg.view(rcnn_batch_size, -1)[ind0:ind1] * \
                           fg_mask.unsqueeze(dim=-1)[ind0:ind1].float()).sum() /\
                           max(fg_sum, 1)
                loss_ew = loss_ew.reshape(1)
                if batch_id == 0:
                    rcnn_loss_reg_ew = loss_ew
                else:
                    rcnn_loss_reg_ew = torch.cat((rcnn_loss_reg_ew, loss_ew))
            ...
            if loss_cfgs.CORNER_LOSS_REGULARIZATION and fg_sum > 0:
                ...
                for batch_id in range(gt_boxes3d_ct.shape[0]):
                    ind0, ind1 = gt_boxes3d_ct.shape[1] * batch_id, gt_boxes3d_ct.shape[1] * (batch_id +1)
                    partial_fg_mask = fg_mask[ind0:ind1]
                    if partial_fg_mask.sum() == 0:
                        if batch_id == 0:
                    ...
                    fg_roi_boxes3d = roi_boxes3d[batch_id].view(-1, code_size)[partial_fg_mask]
                    ...
                    loss_corner = loss_utils.get_corner_loss_lidar(
                        rcnn_boxes3d[:, 0:7],
                        gt_of_rois_src[ind0:ind1][partial_fg_mask][:, 0:7]
                    )

                    weighting = partial_fg_mask.sum() / max(fg_mask.sum().float(),1)

                    if batch_id == 0:
                        loss_corner_ew = (loss_corner.mean() * weighting).reshape(1)
                    else:
                        loss_corner_ew = torch.cat((loss_corner_ew, (loss_corner.mean() * weighting).reshape(1)))

                loss_corner_final = loss_corner_ew.sum()
                loss_corner_final = loss_corner_final * loss_cfgs.LOSS_WEIGHTS['rcnn_corner_weight']
                loss_corner_ew = loss_corner_ew * loss_cfgs.LOSS_WEIGHTS['rcnn_corner_weight']

                rcnn_loss_reg += loss_corner_final
                rcnn_loss_reg_ew += loss_corner_ew
                tb_dict['rcnn_loss_corner'] = loss_corner_final.item()
        else:
            raise NotImplementedError
    ...
    def get_box_cls_layer_loss(self, forward_ret_dict, num_voxels_per_example):
        ...
        batch_size = int(len(batch_loss_cls) / num_voxels_per_example.shape[0])
        for batch_id in range(num_voxels_per_example.shape[0]):  
            ind0, ind1 = batch_size * batch_id, batch_size * (batch_id+1) 

            loss_ew = (batch_loss_cls[ind0:ind1] * \
                       cls_valid_mask[ind0:ind1]).sum() / \
                       (torch.clamp(cls_valid_mask.sum(), min=1.0))

            loss_ew = loss_ew.reshape(1)

            if batch_id == 0:
                rcnn_loss_cls_ew = loss_ew
            else:
                rcnn_loss_cls_ew = torch.cat((rcnn_loss_cls_ew, loss_ew))

        rcnn_loss_cls = rcnn_loss_cls * loss_cfgs.LOSS_WEIGHTS['rcnn_cls_weight']
        rcnn_loss_cls_ew = rcnn_loss_cls_ew * loss_cfgs.LOSS_WEIGHTS['rcnn_cls_weight']
    ...
    def get_loss(self, num_voxels_per_example, tb_dict=None):
