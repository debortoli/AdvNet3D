...
from backbone_module import Pointnet2Backbone, PointconvBackbone
...
class VoteNet(nn.Module):
    r"""
        ...
        network_size: (default: normal)
            Amt of params to use (small, normal, may add large later)
        backbone_network: (default: none)
            pointconv or pointnet
        downsample
            Whether or not to use downsampling (i.e. pooling) when processing the cloud
            This can be turned off to avoid destroying the fine details of smaller objs
        cloud_npoints: int
    ...
    def __init__(self, num_class, num_heading_bin, num_size_cluster, mean_size_arr,
                 backbone_network, downsample, cloud_npoints, input_feature_dim=0, num_proposal=128, vote_factor=1, 
                 sampling='vote_fps', network_size='normal', backbone_layer_type='sa', 
                 use_adversarial_discriminator=False, use_adversarial_pointdan=False):
        super().__init__()
        ...
        if backbone_network == 'pointnet':
            self.backbone_net = Pointnet2Backbone(network_size=network_size,
                                                  backbone_layer_type = backbone_layer_type,
                                                  input_feature_dim=self.input_feature_dim, 
                                                  downsample=downsample,
                                                  cloud_npoints= cloud_npoints,
                                                  use_adversarial_discriminator=self.use_adversarial_discriminator)
        ...
        # adversarial discriminator classification
        if self.use_adversarial_discriminator:
            self.adv_classification_net = AdvClassDiscriminator()
    ...
    def forward(self, inputs):
        ...
        end_points['inp_data_type'] = inputs['data_type']
        ...
        if self.use_adversarial_discriminator:
            end_points = self.adv_classification_net(end_points)
        return end_points
    ...
    def update_using_adv_discriminator(self, value):
        ...
        self.backbone_net.use_adversarial_discriminator = value
...
if __name__=='__main__':
    ...
    try:
        ...
        dump_results(end_points, 'tmp', DC, 1.0, 'votenet_example')
    except:
